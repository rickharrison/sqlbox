...
function save(model, obj, where, mainCallback) {
  ...
  async.series({
    beginTxn: function (callback) {
      begin(model.client, function (err, txn) {
        transaction = txn;
    ...
    saved: function (callback) {
      ...
      if (clone.id) {
        saveUpdate(model, clone, where, transaction, callback);
      } else {
        saveNew(model, clone, transaction, callback);
      }
    },
    commitTxn: function (callback) {
 ...
 */
function saveNew(model, obj, transaction, callback) {
  ...
  transaction.query(query.text, query.values, function saveNew_(err, result) {
    if (model.logQueries) {
 ...
 */
function saveUpdate(model, obj, where, transaction, callback) {
  where = where || {}
  where.id = obj.id;
  ...
  transaction.query(query.text, query.values, function saveUpdate_(err, result) {
    if (model.logQueries) {
